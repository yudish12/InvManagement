...
const formatFormData = (order) => {
  const _order = cloneDeep(order);
  _order.bill_date = parseISO(order.bill_date);
  _order.invoiceNo = order.invoice_number;
  _order.credit_period_radio = [0, 30, 60, 90].includes(order.credit_period)
    ? String(order.credit_period)
    : "custom";
  _order.credit_period = String(order.credit_period);
  _order.inward_date = new Date();
  _order.warehouse = order.warehouseDetails;

  if (Array.isArray(_order.items)) {
    for (let i = 0; i < _order.items.length; i++) {
      _order.items[i].serialNos = "";
      _order.items[i].price = String(_order.items[i].price);
      _order.items[i].qty = String(_order.items[i].qty);
      _order.items[i].warranty_period_radio = [0, 12, 18, 24, 36, 60].includes(
        _order.items[i].warranty_period
      )
        ? String(_order.items[i].warranty_period)
        : "custom";
      _order.items[i].warranty_period = String(_order.items[i].warranty_period);
    }
  }
...
function EditOrderButton({ order, refreshList }) {
  ...
  const [formData, setFormData] = useState(formatFormData(order));
  ...
  const handleClose = (e, reason) => {
    ...
    setFormData(formatFormData(order));
    setOpen(false);
  };
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post("/orders/edit/purchase", _formData);
      dispatch(addSuccess());
      handleClose();
  ...
  const isFormDataValid = () => {
    ...
    if (
      !credit_period ||
      !/\d+/.test(credit_period) ||
      parseInt(credit_period) < 0
    ) {
      dispatch(
        addError(
          generateFormError(
            "CREDIT_PERIOD_INVALID",
            "credit_period",
            "Credit period is invalid"
          )
        )
      );
      return false;
    ...
    if (!mode) {
      dispatch(
        addError(
          generateFormError("MODE_REQUIRED", "mode", "Mode is a required field")
        )
      );
      return false;
    }
      ...
      dispatch(
        addError(
          generateFormError(
            "DOCKETNO_REQUIRED",
            "docket_number",
            "Docket number is a required field"
          )
        )
      );
      return false;
    ...
    if (mode === "inward" && (!inward_date || isNaN(inward_date))) {
      dispatch(
        addError(
          generateFormError(
            "INWARD_DATE_REQUIRED",
            "inward_date",
            "Inward date is a required field"
          )
        )
      );
      return false;
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        dispatch(
          addError(
            generateFormError(
              "QTY_REQUIRED",
              `qty-${i}`,
              "Quantity is invalid, it must be positive number without decimals"
            )
          )
        );
        return false;
        ...
        dispatch(
          addError(
            generateFormError(
              "PRICE_REQUIRED",
              `price-${i}`,
              "Price is invalid, it must be positive number"
            )
          )
        );
        return false;
      ...
      if (
        !warranty_period ||
        !/\d+/.test(warranty_period) ||
        parseInt(warranty_period) < 0
      ) {
        dispatch(
          addError(
            generateFormError(
              "WARRANTY_PERIOD_INVALID",
              `warranty_period-${i}`,
              "Warranty period is invalid"
            )
          )
        );
        return false;
      ...
      if (mode === "inward") {
          ...
          dispatch(
            addError(
              generateFormError(
                "SERIALNO_REQUIRED",
                `serialNos-${i}`,
                "Serial numbers is a required field"
              )
            )
          );
          return false;
        ...
        if (serialNoArr.length !== parseInt(qty)) {
          dispatch(
            addError(
              generateFormError(
                "SERIALNO_COUNT_MISMATCH",
                `serialNos-${i}`,
                "Serial numbers count does not match quantity"
              )
            )
          );
          return false;
        }
        const duplicateSerialNoArr = serialNoArr.filter(
          (item, index) => serialNoArr.indexOf(item) !== index
        );
          ...
          dispatch(
            addError(
              generateFormError(
                "SERIALNO_DUPLICATES",
                `serialNos-${i}`,
                `Serial numbers contain blank value(s)`
              )
            )
          );
          return false;
  ...
  const onChange = (e) => {
    ...
    if (
      e.target.name === "credit_period_radio" &&
  ...
  const onChangeLineItem = (key) => (e) => {
    if (
      ["qty", "warranty_period"].includes(e.target.name) &&
      !isNaN(parseInt(e.target.value)) &&
      parseInt(e.target.value) < 0
    ) {
      return;
    ...
    setFormData((formData) => {
      ...
      const _items = cloneDeep(formData.items);
      if (
        e.target.name === "warranty_period_radio" &&
        e.target.value !== "custom"
      ) {
      ...
      >
        ...
        <DialogContent>
          <DialogContentText sx={{ mb: 3 }}>
            To edit a purchase order, enter below form details and click submit
          </DialogContentText>
              ...
              <DatePicker
                ...
                slotProps={{
                  textField: {
                    ...
                    helperText:
                      error?.field === "bill_date" && error?.message
                        ? error.message
                ...
                onChange={(newDate) =>
                  onChange({ target: { name: "bill_date", value: newDate } })
                }
              />
          ...
          <Box sx={{ mb: 2 }}>
            <FormControl>
              ...
              <RadioGroup
                row
                aria-labelledby="credit_period_radio"
                value={credit_period_radio}
                onChange={onChange}
                name="credit_period_radio"
              >
                <FormControlLabel
                  value="0"
                  control={<Radio />}
              ...
              <LocalizationProvider dateAdapter={AdapterDateFns}>
                <DatePicker
                  ...
                  slotProps={{
                    textField: {
                      helperText:
                        error?.field === "inward_date" && error.message
                          ? error.message
                          : "Date is in YYYY-MM-DD format",
                  ...
                  onChange={(newDate) => {
                    onChange({
                      target: { name: "inward_date", value: newDate },
                    });
                  }}
                />
              </LocalizationProvider>
          ...
          <FormAsyncSelect
            ...
            onChange={(e, option) =>
              setFormData((formData) => ({ ...formData, warehouse: option }))
            }
            value={warehouse}
            ...
            return (
              <Box component="fieldset" sx={{ p: 2, mb: 2 }} key={key}>
                ...
                <FormAsyncSelect
                  ...
                  onChange={(e, option) =>
                    onChangeLineItem(key)({
                      target: { name: "partcode", value: option },
                    })
                  }
                  value={partcode}
                  ...
                  helperText={
                    error?.field === `partcode-${key}` && error?.message
                  }
                ...
                <Box sx={{ my: 2 }}>
                  <FormControl>
                    <FormLabel id="warranty_period_radio">
                      Warranty Period
                    </FormLabel>
                    <RadioGroup
                  ...
                  {warranty_period_radio === "custom" && (
                    <TextField
                      ...
                      helperText={
                        error?.field === `warranty_period-${key}` &&
                        error?.message
                      }
                      disabled={requesting}
                ...
                <Box sx={{ marginTop: 2 }}>
                  {mode === "inward" && (
                    <TextField
                      ...
                      helperText={
                        error?.field === `serialNos-${key}` && error?.message
                      }
                      disabled={requesting}
                  ...
                  {key > 0 && (
                    ...
                    >
                      <Button
                        variant="outlined"
                        color="error"
                        onClick={(e) => handleDeleteLineItem(key)}
                      >
