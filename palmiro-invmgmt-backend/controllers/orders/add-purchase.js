...
const add = async (req, res) => {
  const { account, comments, docket_number, invoiceNo, bill_date, credit_period, mode, inward_date, warehouse, items } = req.body;
  ...
  createObj.docket_number = "";
  if (mode === "intransit") {
    if (docket_number && typeof docket_number === "string" && docket_number.trim().length > 0) {
      createObj.docket_number = docket_number;
    }
  }
  ...
  if (inventories.length > 0) {
    ...
    for (let i = 0; i < inventories.length; i++) {
      inventories[i].purchase_order = getObjectIdFromString(order._id);
    }
